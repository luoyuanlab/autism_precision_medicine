readAutisml2ClusterGTFromVABatch <- function(suids, fnfe="./base_data/filtered_exons.RData", multiplex=F, dp.thr=NA, version='2018') {
    ...
    source("knownVarSanityCheckVA.R")
    ...
    coex_ids = as.numeric(names(cmembership))
    ...
    if (multiplex) {
        ...
        dn.mgt = dn.mgt.l2.multi
        dn.mapping = dn.mapping.l2.multi
    }else {
        dn.va = sprintf('%s/vaRData_uw', dn.ndar)
        dn.mgt = dn.mgt.l2.uw
        dn.mapping = dn.mapping.l2.uw
    }
    if (version=='2018') {
        dn.va = sprintf('%s_2018', dn.va)
        dn.mgt = sprintf('%s_%s', dn.mgt, version)
        dn.mapping = sprintf('%s_%s', dn.mapping, version)
    }
    hva = hash()
    mGT = NULL
    gids = NULL
    ...
    for (suid in suids) {
        ...
        knownVarSanityCheckVA(va[gt!="0/0" & gt!="./.",], suid)
    }
    ...
    for (suid in suids) {
        ...
        submap = cbind(mappedVcfids, coex_ids[subjectHits(h)], cmembership[as.character(coex_ids[subjectHits(h)])], as.character(rd[queryHits(h)]$mcols.Gene.refGene))
