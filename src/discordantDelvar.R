...
dngt = sprintf('%s/mGT_uw', dn.ndar)
dnmapping = sprintf('%s/vaMapping_uw', dn.ndar)
dnlut = sprintf('%s/ssc_lookup_tab', dn.ndar)
fn.delvar = 'autism/discordant_delvar_exon_cluster.txt'

if (version=='2018') {
    dngt = sprintf('%s_%s', dngt, version)
    dnmapping = sprintf('%s_%s', dnmapping, version)
    fn.delvar = sprintf('autism%s/discordant_delvar_exon_cluster.txt', version)
}
